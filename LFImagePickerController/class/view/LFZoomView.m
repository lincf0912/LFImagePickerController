...
@property (nonatomic, weak) LFClippingView *scrollView;
...
- (void)customInit
{
    ...
    LFClippingView *scrollView = [[LFClippingView alloc] initWithFrame:self.bounds];
...
- (void)setImage:(UIImage *)image
{
    ...
    CGRect cropRect = AVMakeRectWithAspectRatioInsideRect(image.size, self.scrollView.frame);
    [self setPreviewRect:cropRect];
}

- (void)setPreviewRect:(CGRect)pRect
{
    self.scrollView.frame = self.bounds;
    self.zoomingView.frame = pRect;
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    if (view == self) { /** 当scrollView小于全屏大小 点击超出部分仍然滚到scrollView */
        return self.scrollView;
    } else if (view == self.zoomingView) { /** 不触发下一层UI响应 */
        return self.scrollView;
    }
    return view;
}
...
- (void)refreshImageZoomViewCenter {
    ...
    self.zoomingView.center = CGPointMake(_scrollView.contentSize.width * 0.5 + offsetX, _scrollView.contentSize.height * 0.5 + offsetY);
}
